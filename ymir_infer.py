    ...
    for idx in range(len(lines)):
        if idx % monitor_gap == 0:
            write_ymir_monitor_process(ymir_cfg,
                                task='infer',
                                naive_stage_percent=idx ,
                                stage=YmirStage.TASK)
        line = lines[idx]
        image = line.strip()
        image_name = osp.basename(image)
        anns = []
        results = model.predict(image,imgsz=imgsz,conf=ymir_cfg.param.conf_thres,device=device,iou=ymir_cfg.param.conf_thres.iou_thres)
        ...
        for index in range(len(boxes)):
            ...
            ann = rw.Annotation(class_name=ymir_cfg.param.class_names[int(boxes[index].cls)],
                                            score=conf,
                                            box=rw.Box(x=int(x), y=int(y), w=int(w),
                                                       h=int(h)))
            anns.append(ann)
